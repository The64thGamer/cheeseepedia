+++
title = "Winchester Pizza Time Orchestra (Animatronic)"
draft = false
type = "Animatronics"
tags = ["Pizza Time Theatre"]
startDate = "1977-00-00"
endDate = "0000-00-00"
contributors = ["Pizza Entertainment Fan", "The 64th Gamer", "ThatRockAfireFan", "StarToday77"]
pageThumbnailFile = "ahX5A2M5qbQTOMLREVPd.avif"
unitsProduced = "1"
manufacturer = "Fantasy Forest"
citations = ["https://www.si.edu/media/NMAH/NMAH-AC1498_Transcript_NolanBushnell.pdf", "https://www.showbizpizza.com/info/promo/ptt/ptt_79franchise.pdf"]
+++
The ***Winchester Pizza Time Orchestra ***was an animatronic produced by {{< wiki-link "Fantasy Forest" >}} in 1977 as an animatronic version of the Pizza Time Orchestra at the first PTT on {{< wiki-link "San Jose, CA (370 S Winchester Blvd)" >}}.

## History

The initial inspiration for the Pizza Time Orchestra would come between 1970-1971, when Nolan Bushnell would visit the local *Pizza & Pipes* restaurant in Redwood City for inspiration on his burgeoning arcade restaurant concept. While the visit mainly inspired the need to have entertainment alongside the food, the mechanically operated instruments would later make their way into the first location in 1977, being placed on shelves around the showroom.{{< cite 1 >}}
The setup would feature it's own dedicated track on the 1977 {{< wiki-link "Pizza Time Theatre Showtape" >}}, and would also appear in the {{< wiki-link "PTT Franchise Brochure (1979)" >}}, were they are referred as Drums & 'Noisemakers'. They were either removed when the store received {{< wiki-link "Cyberamics" >}} or when it store closed in February of 1985.{{< cite 2 >}}

## Instruments

South Wall:

* Tom Drum
* Snare Drum
* Cymbal
* "Timmy" Triangle
* Bass Drum
* "Woody" Woodblock
* "Teddy" Tamborine

East Wall:

* The "Skillet Sextet"
* Various Cups
* Various Kettles
* "Wally" Washboard
* "Chubby" Churn
* "Speedy" Spoons

Along with the aforementioned instruments, a Coffee Grinder & a (lighted) Oil Lamp were also placed on the East Wall shelves.