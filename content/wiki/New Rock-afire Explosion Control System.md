...
# **Decoder** - Accepts video input from the VCRs, decodes the control data from the VBI, and provides a switched video output to the Turntable Board to feed the TVs. Custom designed by CEI.
# **Showplay** - Outputs two channels of Pianocorder-format data to drive the character Bit Strippers, and miscellaneous signals to control the Turntable Board. Custom designed by CEI.
...
# **Floppy controller** - Interfaces with the 3.5' floppy disk drive. Custom designed by CEI.
# (Empty slot)
# **Lightpen** - Reads lightpen signals to control the computer. Custom designed by CEI.
