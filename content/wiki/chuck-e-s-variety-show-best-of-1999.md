+++
draft = false
title = "Chuck E's Variety Show - Best of 1999"
startDate = "2000-01-00"
endDate = "2002-12-00"
pageThumbnailFile = "uDsVmoVLrg1XCX13a2WP.avif"
contributors = ["Cyber Snout", "The 64th Gamer", "Ceclife13", "John95", "Cy's Junk at School"]
citations = ["https://archive.org/download/cyberamics_post_93_svhs_signals/January%202000%20CEC-Stage-002.iso|CEC-Stage SVHS w/ Signals"]
credits = ["Duncan Brannan|Voice of Chuck E. Cheese, Larry The Technician", "Karlisa Burleson|Voice of Helen Henny", "John Bowen|Voice of Mr. Munch, Show Announcer", "Earl Fisher|Voice of Pasqually The Chef", "Jeremy Blaido|Director, Voice of Jasper T. Jowls", "Robert Gotcher|Production Coordinator", "Brian Hagan|Studio Manager", "Brent Gray|Studio Coordinator"]
mediaDuration = "1:31:00"
showtapeFormats = ["Studio C 3x DVD + Floppy", "R12 SVHS (Post-93)", "3-Stage SVHS (Post-93)"]
downloadLinks = ["https://archive.org/download/studio_c_showtape_dvds/January%202000%20Studio%20C-005.iso|Studio C DVD ISO (Disc 1 / 2)", "https://archive.org/download/variety-disc-3/Variety%20Disc%203.iso|Studio C DVD ISO (Disc 3)", "https://archive.org/download/cyberamics_post_93_svhs_signals/January%202000%20CEC-Stage-002.iso|Post '93 CEC-Stage SVHS w/ Signals"]
tags = ["Showtapes"]
categories = ["Chuck E. Cheese's"]
+++
**Chuck E.'s Variety Show** was a showtape initially released in January 2000, which played normally through March 2000. This show was considered generic and could be used as a backup show until 2003.
This show is primarily composed of material from 1997 to 1999, with a few intermissions from 1994 as well.

## Show List:

1. **Segment 1**
    1.  'Chuck E.'s Dance Party'
2. **Intermission 1**
    1.  Will Frye The Nutrition Guy
3. **Segment 2**
    1.  'Dancin' and Singin''
4. **Intermission 2**
    1.  Laugh Tracks
    2.  The Announcer
    3.  CEC TV ID - 'Cows'
5. **Segment 3**
    1.  'Disco Chuck' (Parody of ['Disco Duck'](https://en.wikipedia.org/wiki/Disco_Duck))
6. **Intermission 3**
    1.  Singing Cowboy
7. **Segment 4**
    1.  'Safety Pin' (Parody of ['The Safety Dance'](https://en.wikipedia.org/wiki/The_Safety_Dance))
8. **Intermission 4**
    1.  A Day At The Movies
    2.  Drawing Chuck E.
    3.  CEC TV ID - 'Cows'
9. **Segment 5**
    1.  ['Rockin' Robin'](https://en.wikipedia.org/wiki/Rockin%27_Robin_(song))
10. **Intermission 5**
    1.  Chuck E.'s Big Bowling Date
11. **Segment 6**
    1.  'Tokens To Ride' (Parody of ['Ticket To Ride'](https://en.wikipedia.org/wiki/Ticket_to_Ride_(song)))
12. **Intermission 6**
    1.  Introducing Larry The Technician
    2.  Ratings
    3.  CEC TV ID - 'Cows'
13. **Segment 7**
    1.  'Hip Mouse' (Parody of ['Brick House'](https://en.wikipedia.org/wiki/Brick_House_(song)))
14. **Intermission 7**
    1.  Larry The Technician - 'Curtains'
15. **Segment 8**
    1.  'Pizza Serenade'
16. **Intermission 8**
    1.  Super Jasper - 'Cadillac Ranch'
    2.  Pondwatch
    3.  CEC TV ID - 'Cows'
17. **Segment 9**
    1.  'Chuck E. Town' (Parody of ['Funky Town'](https://en.wikipedia.org/wiki/Funkytown))
18. **Intermission 9**
    1.  Super Jasper - 'Staring Contest'
19. **Segment 10**
    1.  'Let's Dance'
20. **Intermission 10**
    1.  Memories Are Forever
    2.  The Virtuoso
    3.  CEC TV ID - 'Cows'
21. **Segment 11**
    1.  'Cheese Pizza Is Pure Delight' (Parody of ['Cheeseburger in Paradise'](https://en.wikipedia.org/wiki/Cheeseburger_in_Paradise))
    2.  'Boat Of Love'
    3.  'We Sail The Ocean Blue'
22. **Intermission 11**
    1.  Name That Cheese
    2.  Larry The Technician - 'Video Feed'
    3.  CEC TV ID - 'Cows'
23. **Segment 12**
    1.  'We All Love Music'
24. **Intermission 12**
    1.  Clowns Making a Sandwich
25. **Segment 13**
    1.  'We Are The Same'
26. **Intermission 13**
    1.  I Like This Game
    2.  Baking With Betty - 'Vegetable Medley'
    3.  CEC TV ID - 'Cows'
    4.  Book Your Party Online (Existing Stages)


## Live Shows:

**Live shows would be on separate VHS tapes for Existing Stage stores.**

- Celebration 2000
- Birthday Star '98
- Spanish Birthday
- Chuck E. March
- Havin' A Party at Chuck E.'s Place
- Chuck E's Fiesta Mexicana
- Chuck E. Says
- Chuck E. Shuffle
- Patriotic '98
- America '99
- Chuck E. Swing
- Rockin' Christmas Eve
- Give My Regards to Broadway (Existing Stages)

**Birthday Break**: Chuck E.'s Variety Show logo with music set to a 1-minute, 2-minute, or 3-minute countdown leading to the birthday show.

## Trivia:

- This show replaces {{< wiki-link "Best of CEC TV 1993" >}} as the emergency backup show for Existing Stages.
- 'A Day At The Movies' is reused from the {{< wiki-link "Patriotic 1994 Show" >}}. 'Laugh Tracks', 'Chuck E.'s Big Bowling Date' and 'The Virtuoso' are reused from {{< wiki-link "September '94" >}}. 'Will Frye The Nutrition Guy', 'The Announcer', 'Pondwatch', 'Name That Cheese', and Larry The Technician - 'Video Feed' are reused from {{< wiki-link "Studio C Premiere" >}}. 'Introducing Larry The Technician' and 'I Like This Game' are reused from {{< wiki-link "September 1998" >}}. 'Disco Chuck', 'Safety Pin', 'Rockin' Robin', and 'Tokens To Ride' are reused from {{< wiki-link "January 1999" >}}. 'Chuck E.'s Dance Party', 'Dancin' and Singin'', 'Singing Cowboy', 'Ratings',   'Hip Mouse', 'Pizza Serenade', Super Jasper - 'Cadilac Ranch', 'Chuck E. Town', Super Jasper - 'Staring Contest', 'Let's Dance', and 'Memories Are Forever' are reused from {{< wiki-link "April 1999" >}}. 'Drawing Chuck E.', 'Cheese Pizza Is Pure Delight', Boat of Love', 'We Sail The Ocean Blue', 'We All Love Music', 'Clowns Making A Sandwich', 'We Are The Same', and 'Baking With Betty' are reused from {{< wiki-link "August 1999" >}}.
- This is the first show to feature the 'Baking with Betty' and 'Clowns Making a Sandwich' intermissions on Existing Stages. Previously, they were only on the Studio C version of {{< wiki-link "August 1999" >}}.
- The Studio C puppet backgrounds are reused from {{< wiki-link "August 1999" >}}. All puppet footage was rerecorded for this show.
- The bluescreen backgrounds for Studio C are reused from their original shows.
- Segment 11 is split into two segments on Studio C. 
- 'Cheese Pizza Is Pure Delight' and 'Hip Mouse' are both sung by Chuck E. on Studio C, just like in their original shows. Chuck E.'s vocals in 'Pizza Serenade', which were previously exclusive to Studio C, are featured in all versions of this show.
