...
export function initSearch(app) {
  (async () => {
    ...
    function buildReviewHTML(doc, meta, body, showFull) {
      ...
      const pageId = page ? ARTICLE_LINKER[page] : null;
      const pageHref = pageId ? `/?v=cep-js&=${encodeURIComponent(pageId)}` : '/?v=cep-js&=' + encodeURIComponent(norm(page));
      const pageLink = page ? `<a href="${pageHref}" class="ReviewPage">${esc(page)}</a>` : '';
    ...
    ${doc.d ? `<div class="s-item-meta">${esc(fmtDate(doc.d))}</div>` : ''}
      ...
      fetch(`/content/${doc.p}/meta.json`).then(r => r.json()).catch(() => ({})).then(meta => {
        ...
        const page = meta.page;
        if (page) {
          const pid = ARTICLE_LINKER[page] || null;
          const metaEl = el.querySelector('.s-item-meta');
          if (metaEl) metaEl.insertAdjacentHTML('beforeend', ` · <a href="${pid ? '/?v=cep-js&=' + encodeURIComponent(pid) : '/?v=cep-js&=' + encodeURIComponent(norm(page))}">${esc(page)}</a>`);
        }
      });
      return el;
    }
