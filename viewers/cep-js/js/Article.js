...
async function getTitleIndex() {
  if (!TITLE_INDEX) {
    ...
    console.log('Title index loaded:', Object.keys(TITLE_INDEX).length, 'entries');
    console.log('Sample:', Object.keys(TITLE_INDEX).slice(0,3));
  }
  return TITLE_INDEX;
}
...
async function renderMarkdown(md) {
  ...
  let html = md
    ...
    .replace(/\[([^\]]+)\]/g, (_,title) => {
      const id = idx[title];
      console.log('Link lookup:', title, '->', id);
      return id
