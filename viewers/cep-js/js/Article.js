...
function fmtDate(d){if(!d||d==='0000-00-00'||!d.trim())return '???';const[y,m,day]=d.split('-'),yi=parseInt(y,10),mi=parseInt(m,10),di=parseInt(day,10);if(!yi)return '???';const mn=mi?MNAMES[mi]:'',dn=di?String(di):'',dp=mn+dn;return(dp?dp+(dp.endsWith(' ')?'':' '):'')+yi;}
function fmtDateRange(s,e){const start=fmtDate(s);if(e===undefined||e===null||e==='')return start+' - Present';if(!e||e==='0000-00-00')return start+' - ???';return start+' - '+fmtDate(e);}
...
async function renderMarkdown(md){
  ...
  let html=md
    ...
    .replace(/\[([^\]]+)\]/g,(_,title)=>{const id=idx[title];return id?`<a href="/?v=cep-js&=${encodeURIComponent(id)}">${title}</a>`:`<span class="BadLink">${title}</span>`;})
...
function buildInfobox(meta){
  ...
  rowList('Tags',meta.tags); rowList('Contributors',meta.contributors);
  rowList('Stages',meta.stages); rowList('Remodels',meta.remodels);
  rowList('Animatronics',meta.animatronics); rowList('Franchisees',meta.franchisees);
  rowList('Attractions',meta.attractions); rowList('Credits',meta.credits);
...
export async function loadArticle(app, articleId){
  ...
  infobox.innerHTML='';
  if(thumbFolder){const imgWrap=document.createElement('div');imgWrap.className='InfoboxThumb';imgWrap.appendChild(ProgressiveImage(thumbFolder,meta.title||''));infobox.appendChild(imgWrap);}
...
const showInbox = show => { infobox.style.display = show ? '' : 'none'; };
  ...
  async function showArticle(){
    ...
    if(articleContribs.length){
      ...
      if(contribEl){
        ...
        const filtered=allUsers.filter(u=>articleContribs.some(c=>c.toLowerCase()===(u.name||'').toLowerCase()||c.toLowerCase()===(u.fu||'').toLowerCase()));
  ...
  if(btnBar){
    ...
    let articleBtn=null;
    if(hasAnySections)articleBtn=makeBtn('Article',showArticle);
