...
def fetch_discourse_users():
    """Fetch user data from Discourse and combine with wiki contributors"""
    ...
    try:
        # Fetch all users from Discourse
        while True:
            ...
            if not data.get('meta', {}).get('load_more_directory_items'):
                break
            page += 1
        ...
        # Update Discourse users with contribution counts
        ...
        # Add "Guest" users for Wiki contributors not on Discourse
        for c_name, c_count in contributor_counts.items():
...
def run():
    """Main entry point for fetching all Discourse data"""
    print("=== Fetching Discourse Data ===")
    fetch_discourse_news()
    fetch_discourse_users()
    print("Discourse data fetch complete!")
